...
const DDCA_TOTAL_SUPPLY = D18.mul(new BN("10000000000"));
...
describe("MiningEco create project", function () {
  ...
  it("create && succeed && claim bonus", async function () {
    ...
    const bonus = await Bonus.deploy(this.miningEco.address, this.DDCA.address);
    ...
    await this.DDCA.connect(admin).mint(facai.toString());
    await this.DDCA.connect(admin).transfer(bonus.address, facai.toString());
    await this.DDCA
      .connect(pm)
      .approve(this.miningEco.address, this.balancePM.toString());
    await this.usdt
    ...
    const pm_balance = await this.DDCA.balanceOf(pm.address);
    await mineBlocks(20);
    ...
    expect(
      ...
      pm_balance.sub(await this.DDCA.balanceOf(pm.address)).toString()
    );

    await bonus.connect(other).claim_investment_bonus(projectId);
    expect((await this.DDCA.balanceOf(other.address)).toString()).to.equal(
      facai.toString()
    );
  ...
  it("change insurance rate by audit", async function () {
    ...
    expect(insurance_should).to.equal(
      pm_balance.sub(await this.DDCA.balanceOf(pm.address))
    );
