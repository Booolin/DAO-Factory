...
const DDCA_TOTAL_SUPPLY = D18.mul(new BN("100000000000000000"));
...
async function main() {
  ...
  const DDCA = await ethers.getContractFactory("StakingToken");
  ...
  {
    await DDCA.attach(addrs.DDCA).connect(owner).mint(balancePM.toString());
    await DDCA.attach(addrs.DDCA)
      .connect(owner)
